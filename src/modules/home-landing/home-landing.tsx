  ...
  return (
    <section className="relative h-[100vh] text-white">
      <Container>
        ...
        >
          {props?.background && <Image loader={imageLoader} {...props.background} width={1000} priority className="absolute object-cover w-full h-full right-0" />}
        </motion.div>
          ...
          <div className="text-white">
            <div className="w-full relative flex flex-col text-left gap-8">
              ...
              <div className="flex gap-6">
                {props?.callToAction1 && <Button className=" text-white ">{props.callToAction1.text}</Button>}
                {props?.callToAction2 && (
                  <Button intent={"outlined"} className=" text-white ">
                    {props.callToAction2.text}
                  </Button>
                )}
              </div>
            </div>
